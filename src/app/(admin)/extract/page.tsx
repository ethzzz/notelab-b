...
export default function ExtractPage() {
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <p className="text-zinc-600 text-sm">给一段文本和目标字段，AI 按字段抽取出结构化 JSON（演示 JSON 模式 / function calling 思想）。</p>
      ...
      {tried && (
        <Card size="small" className="shadow-sm" title={<span className="text-indigo-600">抽取结果（JSON）</span>}>
          <div className="flex flex-col gap-3">
            {result !== null ? (
              <pre className="bg-zinc-900 text-zinc-100 rounded-lg p-4 text-sm overflow-x-auto">{JSON.stringify(result, null, 2)}</pre>
            ) : (
              <div className="text-sm text-zinc-600">（未能解析为 JSON，原始输出见下方）</div>
            )}
            {raw && (
              <div>
                <div className="text-xs text-zinc-400 mb-1">模型原始输出</div>
                <pre className="bg-zinc-50 border border-zinc-200 rounded-lg p-3 text-xs whitespace-pre-wrap break-words">{raw}</pre>
              </div>
            )}
          </div>
        </Card>
