...
function loadMeta(): Meta {
  ...
  try {
    const m = JSON.parse(localStorage.getItem("b_vs-meta") || "")
...
export default function VsPage() {
  ...
  const buy = (id: keyof MetaUpg) => {
    setMeta(m => {
      ...
      localStorage.setItem("b_vs-meta", JSON.stringify(next))
      return next
    })
  }
  ...
  useEffect(() => { setBest(Number(localStorage.getItem("b_vs-best") || 0)) }, [])
      ...
      if (s && phaseRef.current === "play") {
        ...
        if (s.pending > 0) {
          ...
          setBest(b => {
            ...
            localStorage.setItem("b_vs-best", String(nb))
            return nb
          })
