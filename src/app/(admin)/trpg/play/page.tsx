...
export default function TrpgPlayPage() {
  ...
  const loadPlays = useCallback(() => {
    apiJson("/api/trpg/plays").then((j) => setPlays(j.plays || [])).catch(() => {})
  }, [])
  ...
  async function start(id: number) {
    setBusy(true)
    try {
      const j = await apiJson(`/api/trpg/scenarios/${id}/play`, { method: "POST" })
  ...
  async function resume(pid: number) {
    setBusy(true)
    try {
      const j = await apiJson(`/api/trpg/plays/${pid}`)
  ...
  async function choose(c: Choice) {
    ...
    try {
      if (c.dice) {
        ...
        const j: PlayPayload = await apiJson(`/api/trpg/plays/${play.play_id}/choose`, {
      ...
      const j = await apiJson(`/api/trpg/plays/${play.play_id}/choose`, {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ choice_id: c.id }),
      })
      setPlay(j); setPos(j.history?.length || 0); setTypingDone(false)
      loadPlays()
  ...
  async function abandon() {
    ...
    if (!ok) return
    try { await api(`/api/trpg/plays/${play.play_id}`, { method: "DELETE" }) } catch { /* ignore */ }
  ...
  async function removePlay(pid: number) {
    ...
    if (!ok) return
    try { await api(`/api/trpg/plays/${pid}`, { method: "DELETE" }); loadPlays() } catch { /* ignore */ }
  }
  async function openPicker() {
    try {
      const j = await apiJson("/api/trpg/scenarios")
  ...
  return (
    <div className="relative w-full h-[calc(100dvh-140px)] min-h-[560px] rounded-3xl overflow-hidden flex items-center justify-center"
      ...
      <Modal open={pickerOpen} onClose={() => setPickerOpen(false)} title="📚 选择剧本" maxW="max-w-xl">
        ...
        {scenarios.length === 0 && (
          <div className="py-10 text-center text-sm text-zinc-400">剧本库是空的，先去「生成剧本」菜单创作一个吧</div>
        )}
        <div className="flex flex-col gap-2">
          {scenarios.map((s) => (
