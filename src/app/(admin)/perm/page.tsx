...
export default function PermPage() {
  ...
  const columns = [
    { title: "权限码", dataIndex: "code", width: 220, render: (v: string, r: Route) => <span className={`font-mono text-xs ${r.kind === "page" ? "text-indigo-600 dark:text-indigo-300" : "text-zinc-500 dark:text-zinc-400"}`}>{v}</span> },
    { title: "路由", dataIndex: "path", render: (v: string) => <span className="font-mono text-xs">{v}</span> },
    { title: "方法", dataIndex: "method", width: 90, render: (v: string, r: Route) => <span className="text-xs text-zinc-400 dark:text-zinc-500">{r.kind === "page" ? "-" : v}</span> },
    { title: "类型", dataIndex: "kind", width: 80, render: (k: string) => k === "page" ? <Tag color="success">页面</Tag> : <Tag>API</Tag> },
    { title: "名称", dataIndex: "name", render: (v: string, r: Route) => <span className={`text-xs ${r.kind === "page" ? "" : "text-zinc-400 dark:text-zinc-500"}`}>{v}</span> },
  ]
      ...
      <Card size="small" className="shadow-sm border-indigo-200/70 dark:border-indigo-500/30">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <div className="flex-1 min-w-[220px]">
            <div className="font-semibold text-sm text-zinc-800 dark:text-zinc-100">C 端用户管理</div>
            <div className="text-xs text-zinc-400 dark:text-zinc-500">管理游戏中心（C 端）账号与用户组：新建/编辑/重置密码/停用，接口 /api/c-admin/*</div>
          </div>
          <Link href="/c-users">
